...
│   │   ├── concept-vote-store.ts # ConceptVoteStore
...
│   │   ├── in-memory-concept-vote-store.ts
│   │   └── index.ts
...
└── socket/
    ...
    ├── concept-vote-handler.ts # concept-vote:* 이벤트 (ConceptVoteStore 사용, 투표)
...
채팅 이력, 방 목록, 플레이어 세션, 요청사항, 랭킹, 플랜카드, 투표를 Redis에 저장한다.

- 방 목록은 GameManager의 인메모리 Map이 source of truth이고 Redis는 write-through 백업이다
- 채팅, 세션, 요청사항, 랭킹, 플랜카드는 Redis가 primary store이다
- Redis 장애 시 7개 store 모두 인메모리 구현으로 전환하여 전체 기능이 정상 동작한다 (graceful degradation)
...
| 투표 | `concept-vote:{conceptFile}` | SET | 컨셉별 투표자 browserId |
...
│   ├── lobby/concept-vote-panel.tsx # 디자인 컨셉 투표 패널
