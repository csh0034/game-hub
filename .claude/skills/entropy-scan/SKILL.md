...
| 체크포인트 | gomoku | texas-holdem | minesweeper | tetris | liar-drawing |
|---|---|---|---|---|---|
| GameType 유니온 | ✅ | ✅ | ✅ | ✅ | ✅ |
| GAME_CONFIGS | ✅ | ✅ | ✅ | ✅ | ✅ |
| game-manager 등록 | ✅ | ✅ | ✅ | ✅ | ✅ |
...
1. `apps/server/src/storage/index.ts` — `Storage` 인터페이스의 프로퍼티에서 스토어 이름 추출 (chatStore, roomStore, sessionStore, requestStore)
...
- `chat:*` → `lobby-handler.ts` (lobby-handler가 chat 이벤트도 처리 — 프로젝트 컨벤션)
- `request:*` → `request-handler.ts`
- `system:*` → `broadcast-player-count.ts`
...
|---|---|---|---|---|
| lobby | 5 | 7 | lobby-handler.ts | ✅ |
| game | 4 | 5 | game-handler.ts | ✅ |
| player | 2 | 0 | nickname-handler.ts | ✅ |
| chat | 3 | 2 | lobby-handler.ts | ✅ |
| request | 6 | 3 | request-handler.ts | ✅ |
| system | 0 | 1 | broadcast-player-count.ts | ✅ |
```
...
1. `apps/web/src/stores/` — `*-store.ts` 파일 목록에서 기능 영역 추출 (lobby, game, chat, request)
