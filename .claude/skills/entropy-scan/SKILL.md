...
1. `apps/server/src/storage/index.ts` — `Storage` 인터페이스의 프로퍼티에서 스토어 이름 추출 (chatStore, roomStore, sessionStore, requestStore, rankingStore)
...
- `system:*` → `broadcast-player-count.ts` + `announce-handler.ts` (system:player-count는 broadcast-player-count, system:announce/announcement는 announce-handler)
...
| chat | 3 | 2 | chat-handler.ts | ✅ |
| request | 6 | 3 | request-handler.ts | ✅ |
| ranking | 2 | 1 | game-handler.ts | ✅ |
| system | 1 | 2 | broadcast-player-count.ts, announce-handler.ts | ✅ |
```
