...
- **관전 모드** (`docs/spectator.md`): 모든 게임에서 최대 4명까지 관전 가능. 호스트가 "관전 허용" ON/OFF 설정 (기본값 OFF). 관전자는 waiting 상태에서만 입장 가능하며, 게임 종료 후 웨이팅룸에 잔류한다. 관전자 이탈 시 게임이 종료되지 않는다. 비공개 정보(홀덤 홀카드, 라이어 역할/제시어, 캐치마인드 제시어)는 관전자에게 모두 공개된다. 관전자 채팅은 별도 ON/OFF 설정 (기본값 ON).
