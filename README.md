...
| 게임 | 인원 | 설명 |
|------|------|------|
| 🎱 3쿠션 당구 | 1~8명 | 수구로 목적구 2개를 맞히되 쿠션 3회 이상 터치해야 득점, 풀스크린 3D |
| 🧩 노노그램 | 1명 | 행/열 숫자 힌트를 보고 칸을 채워 숨겨진 그림을 완성하는 퍼즐 |
| ⌨️ 타자 게임 | 1~8명 | 위에서 내려오는 단어를 빠르게 입력하여 제거하는 타자 레이스 |
| 🎨 라이어 드로잉 | 3~8명 | 소셜 디덕션 그림 게임, 제시어를 모르는 라이어를 투표로 찾아내기 |
| 🖼️ 캐치마인드 | 3~8명 | 출제자가 그린 그림을 보고 채팅으로 정답을 맞추는 드로잉 퀴즈 게임 |
| 🧱 테트리스 | 1~8명 | 클래식/스피드 레이스(40줄 Sprint) 모드, 7-bag 랜덤, SRS 벽차기, 대전 시 쓰레기 줄 공격 |
...
- **Redis 영속화** — 채팅 이력, 방 목록, 플레이어 세션, 랭킹, 플랜카드를 Redis에 저장하여 서버 재시작 후에도 복구
- **랭킹 시스템** — 지뢰찾기(완료시간)/테트리스 스피드 레이스(클리어시간) 난이도별 Top 10 리더보드, 대기실 및 게임 결과에 표시
...
- **관전 모드** — 모든 게임에서 최대 4명 관전 가능, 호스트가 허용 여부 및 관전자 채팅 ON/OFF 설정
- **강퇴** — 방장이 대기 중 플레이어/관전자를 내보내기 가능, 관전 비활성화 시 관전자 일괄 강퇴
- **요청사항 게시판** — 기능 요청 등록, 관리자 완료 처리(커밋 링크 자동 생성) 및 삭제
- **관리자 도구** — 실시간 공지 전송, 로비 상단 플랜카드(배너) 설정
