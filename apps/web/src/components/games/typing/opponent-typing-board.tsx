...
function MiniFallingWord({ word }: { word: TypingWord }) {
  ...
  return (
    ...
    >
      <span className="inline-block bg-gradient-to-b from-neon-cyan to-neon-purple text-white px-1.5 py-0.5 rounded-full text-[10px] font-medium whitespace-nowrap shadow-[0_0_6px_rgba(0,229,255,0.3)]">
        {word.text}
      </span>
    </div>
  );
...
export const OpponentTypingBoard = memo(function OpponentTypingBoard({
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <div className="relative bg-gradient-to-b from-background to-card border border-border/40 rounded-xl overflow-hidden w-44 h-48 shadow-inner neon-border">
        {/* 위험 구역 */}
        ...
        {isDead && (
          <div className="absolute inset-0 bg-background/85 backdrop-blur-sm flex items-center justify-center z-10">
            <span className="text-sm font-display font-bold text-accent">탈락</span>
          </div>
        )}
      </div>
      <div className="flex items-center justify-between px-1.5 text-[10px]">
        <span className={`font-medium truncate max-w-[70px] ${isDead ? "line-through text-muted-foreground" : "text-foreground/80"}`}>
          {player.nickname}
        </span>
        <div className="flex items-center gap-1.5 tabular-nums shrink-0">
          ...
          {player.combo >= 3 && (
            <span className="text-neon-yellow font-bold">{player.combo}x</span>
          )}
        </div>
      </div>
    </div>
